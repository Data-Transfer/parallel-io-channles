...
/// Error generated by consumers.
#[derive(Clone, Debug)]
pub struct ConsumerError {
    pub msg: String,
}

/// Generic error type containing producer, consumer and other errors.
#[derive(Clone, Debug)]
pub enum WriteError {
...
pub fn write_to_file<T: 'static + Clone + Send>(
    ...
    for h in consumers_handles {
        match h.join() {
            ...
            Err(_err) => {
                return Err(WriteError::Other("Error joining threads".to_string()));
            }
        }
    }
    Ok(bytes_consumed)
                ...
                match cc.call(&mut buffer, &data, offset as u64) {
                    Err(err) => {
                        (0..cfg.consumers.len()).for_each(|c| {
                            let _ = cfg.consumers[c].send(Error(ProducerError{ msg: err.to_string(), offset: offset}));
                        });
                    ...
                    match msg {
                        ...
                        Consume(cfg, buffer) => {
                            ...
                            if let Err(err) = write_bytes_at(&buffer, &file, cfg.offset) {
                                return Err(WriteError::Consumer(
                                    ConsumerError{msg: format!("Cannot write to file - {}", err.to_string())}));
                            }
